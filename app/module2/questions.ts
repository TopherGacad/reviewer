export const questions = [
    {
      id: 1,
      question: "Which type of ionizing radiation is primarily responsible for causing ionization in biological tissues during X-ray procedures?",
      options: [
        "Alpha particles",
        "Gamma rays",
        "X-rays",
        "Beta particles"
      ],
      correctAnswer: 2
    },
    {
      id: 2,
      question: "During a radiologic procedure, the technologist needs to adjust the kilovoltage (kVp). Increasing the kVp results in:",
      options: [
        "Greater image contrast",
        "Higher patient dose",
        "Higher energy X-rays with greater penetration",
        "Lower X-ray beam intensity"
      ],
      correctAnswer: 2
    },
    {
      id: 3,
      question: "Which of the following materials is most commonly used to filter out low-energy X-rays in diagnostic radiology?",
      options: [
        "Lead",
        "Aluminum",
        "Tungsten",
        "Copper"
      ],
      correctAnswer: 1
    },
    {
      id: 4,
      question: "In the event of an X-ray overexposure, which of the following radiation protection principles should be prioritized to minimize patient dose?",
      options: [
        "Increase shielding",
        "Use collimation",
        "Reduce exposure time",
        "Increase distance from the source"
      ],
      correctAnswer: 2
    },
    // {
    //   id: 5,
    //   question: "Which component in the X-ray tube is responsible for the production of electrons through thermionic emission?",
    //   options: [
    //     "Cathode",
    //     "Anode",
    //     "Focusing cup",
    //     "Filament"
    //   ],
    //   correctAnswer: 3
    // },
    // {
    //   id: 6,
    //   question: "What is the purpose of using a grid during radiographic imaging?",
    //   options: [
    //     "To enhance image brightness",
    //     "To reduce patient exposure",
    //     "To decrease scatter radiation and improve image contrast",
    //     "To magnify the image"
    //   ],
    //   correctAnswer: 2
    // },
    // {
    //   id: 7,
    //   question: "The ALARA principle in radiologic protection stands for:",
    //   options: [
    //     "Always Lower Absorbed Radiation Accurately",
    //     "Avoid Long-Range Radiation Accumulation",
    //     "As Low As Reasonably Achievable",
    //     "Avoid Lateral Radiographic Applications"
    //   ],
    //   correctAnswer: 2
    // },
    // {
    //   id: 8,
    //   question: "Which interaction between X-rays and matter primarily contributes to image formation in diagnostic radiography?",
    //   options: [
    //     "Compton scattering",
    //     "Photoelectric effect",
    //     "Pair production",
    //     "Coherent scattering"
    //   ],
    //   correctAnswer: 1
    // },
    // {
    //   id: 9,
    //   question: "Which of the following best describes ionizing radiation?",
    //   options: [
    //     "It causes thermal energy absorption",
    //     "It transfers visible light to materials",
    //     "It is capable of removing an orbital electron from an atom",
    //     "It generates non-harmful electromagnetic waves"
    //   ],
    //   correctAnswer: 2
    // },
    // {
    //   id: 10,
    //   question: "A technologist notices that image blur occurs during a radiographic examination. Which factor is most likely to contribute to this?",
    //   options: [
    //     "High kVp",
    //     "Low mA",
    //     "Long exposure time",
    //     "Use of a grid"
    //   ],
    //   correctAnswer: 2
    // },
    // {
    //   id: 11,
    //   question: "The largest contributor to man-made ionizing radiation exposure in the general population is:",
    //   options: [
    //     "Nuclear power plants",
    //     "Diagnostic X-rays",
    //     "Radiation therapy",
    //     "Air travel"
    //   ],
    //   correctAnswer: 1
    // },
    // {
    //   id: 12,
    //   question: "Which of the following is considered a non-ionizing form of electromagnetic radiation?",
    //   options: [
    //     "X-rays",
    //     "Gamma rays",
    //     "UV light",
    //     "Visible light"
    //   ],
    //   correctAnswer: 3
    // },
    // {
    //   id: 13,
    //   question: "In radiographic practice, what is the function of the collimator?",
    //   options: [
    //     "To adjust the intensity of the X-ray beam",
    //     "To focus the X-ray beam on the area of interest",
    //     "To limit the size and shape of the X-ray beam",
    //     "To amplify the X-ray energy for higher resolution"
    //   ],
    //   correctAnswer: 2
    // },
    // {
    //   id: 14,
    //   question: "Which scientist is associated with the development of the moving grid used to reduce scatter radiation?",
    //   options: [
    //     "Gustav Bucky",
    //     "Wilhelm Roentgen",
    //     "Hollis Potter",
    //     "Thomas Edison"
    //   ],
    //   correctAnswer: 2
    // },
    // {
    //   id: 15,
    //   question: "A radiologic technologist is working with a pregnant patient. To minimize fetal radiation exposure, the technologist should:",
    //   options: [
    //     "Use higher kVp and lower mA",
    //     "Avoid imaging the pelvis and lower abdomen, especially in the first trimester",
    //     "Increase exposure time to capture a better image",
    //     "Use a Potter-Bucky grid to reduce scatter"
    //   ],
    //   correctAnswer: 1
    // },
    // {
    //   id: 16,
    //   question: "The primary advantage of using a radiographic intensifying screen is to:",
    //   options: [
    //     "Increase image sharpness",
    //     "Increase patient exposure",
    //     "Reduce patient exposure to radiation",
    //     "Increase the image contrast"
    //   ],
    //   correctAnswer: 2
    // },
    // {
    //   id: 17,
    //   question: "In radiology, the energy of X-rays is measured in:",
    //   options: [
    //     "Volts",
    //     "Watts",
    //     "Electron volts (eV)",
    //     "Joules"
    //   ],
    //   correctAnswer: 2
    // },
    // {
    //   id: 18,
    //   question: "Which device in radiology serves to convert light energy into electrical signals for image formation?",
    //   options: [
    //     "Cathode",
    //     "Anode",
    //     "Photomultiplier tube",
    //     "Collimator"
    //   ],
    //   correctAnswer: 2
    // },
    // {
    //   id: 19,
    //   question: "What is the most effective way to reduce exposure to scatter radiation for both the patient and radiologic technologist?",
    //   options: [
    //     "Using lead aprons",
    //     "Increasing the kVp",
    //     "Reducing the distance between the patient and the X-ray source",
    //     "Using proper collimation and shielding"
    //   ],
    //   correctAnswer: 3
    // },
    // {
    //   id: 20,
    //   question: "Which of the following types of energy are released during a chemical reaction?",
    //   options: [
    //     "Kinetic energy",
    //     "Chemical energy",
    //     "Thermal energy",
    //     "Electromagnetic energy"
    //   ],
    //   correctAnswer: 1
    // },
    // {
    //     id: 21,
    //     question: "X-ray photons are primarily produced by which process in the X-ray tube?",
    //     options: [
    //       "Photoelectric absorption",
    //       "Bremsstrahlung radiation",
    //       "Compton scattering",
    //       "Pair production"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 22,
    //     question: "Which type of radiation is used in the treatment of certain cancers due to its high penetrating power and ionizing capability?",
    //     options: [
    //       "Visible light",
    //       "X-rays",
    //       "Gamma rays",
    //       "Microwaves"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 23,
    //     question: "What is the typical annual dose of radiation from natural environmental sources for an average person?",
    //     options: [
    //       "50 mrem/year",
    //       "300 mrem/year",
    //       "600 mrem/year",
    //       "1000 mrem/year"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 24,
    //     question: "Who is credited with inventing the fluoroscope?",
    //     options: [
    //       "Wilhelm Roentgen",
    //       "Thomas Edison",
    //       "Gustav Bucky",
    //       "William Rollins"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 25,
    //     question: "Which of the following best describes the photoelectric effect?",
    //     options: [
    //       "X-rays passing through the patient without interaction",
    //       "Complete absorption of the X-ray photon by an atom",
    //       "Scattering of the X-ray photon with partial energy loss",
    //       "Creation of an electron-positron pair"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 26,
    //     question: "The image receptor in digital radiography primarily functions to:",
    //     options: [
    //       "Capture scattered X-rays",
    //       "Convert X-ray energy into a digital image",
    //       "Produce light from X-ray photons",
    //       "Measure the radiation dose to the patient"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 27,
    //     question: "When a technologist increases the SID (source-to-image distance), what effect does this have on the image?",
    //     options: [
    //       "Increases magnification",
    //       "Reduces patient dose",
    //       "Increases image contrast",
    //       "Reduces image sharpness"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 28,
    //     question: "Who developed the Coolidge X-ray tube, which is still the basis for modern X-ray tubes?",
    //     options: [
    //       "Thomas Edison",
    //       "Wilhelm Roentgen",
    //       "William D. Coolidge",
    //       "Michael Pupin"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 29,
    //     question: "Which of the following is the best practice when examining a patient using ionizing radiation?",
    //     options: [
    //       "Never use gonadal shielding",
    //       "Always use the largest field size for convenience",
    //       "Use collimation and shielding to limit radiation exposure",
    //       "Increase the exposure time for better image quality"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 30,
    //     question: "Which factor contributes most to the development of image blur?",
    //     options: [
    //       "Use of high kVp",
    //       "Long exposure time",
    //       "Low patient exposure",
    //       "Use of collimation"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 31,
    //     question: "The primary purpose of using lead-lined protective apparel in radiology is to:",
    //     options: [
    //       "Increase image sharpness",
    //       "Reduce scatter radiation",
    //       "Prevent direct radiation exposure",
    //       "Improve image contrast"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 32,
    //     question: "Which of the following increases the likelihood of the photoelectric effect occurring during a radiologic exam?",
    //     options: [
    //       "High kVp",
    //       "Low atomic number of the target material",
    //       "Low kVp",
    //       "Use of non-ionizing radiation"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 33,
    //     question: "The SI unit for radiation absorbed dose is the:",
    //     options: [
    //       "Sievert",
    //       "Gray",
    //       "Rad",
    //       "Rem"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 34,
    //     question: "Which process is most responsible for generating X-rays in diagnostic radiology?",
    //     options: [
    //       "Compton scattering",
    //       "Bremsstrahlung radiation",
    //       "Coherent scattering",
    //       "Pair production"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 35,
    //     question: "What is the main purpose of using beam collimation during radiographic exams?",
    //     options: [
    //       "To increase patient exposure",
    //       "To focus the X-ray beam",
    //       "To limit the radiation field and reduce scatter",
    //       "To amplify the radiation signal"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 36,
    //     question: "Which interaction between X-rays and matter results in the creation of an ion pair?",
    //     options: [
    //       "Compton scattering",
    //       "Coherent scattering",
    //       "Photoelectric effect",
    //       "Pair production"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 37,
    //     question: "In fluoroscopy, which device is used to amplify light for better image quality?",
    //     options: [
    //       "Grid",
    //       "Intensifier",
    //       "Collimator",
    //       "Filtration"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 38,
    //     question: "Which element is most commonly used for X-ray filtration due to its ability to absorb low-energy photons?",
    //     options: [
    //       "Copper",
    //       "Lead",
    //       "Aluminum",
    //       "Zinc"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 39,
    //     question: "The energy of X-ray photons is typically expressed in:",
    //     options: [
    //       "Amps",
    //       "Volts",
    //       "Electron volts (eV)",
    //       "Joules"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 40,
    //     question: "What is the primary purpose of the grid in radiology?",
    //     options: [
    //       "To increase patient exposure",
    //       "To reduce scatter radiation",
    //       "To enhance the X-ray beam intensity",
    //       "To focus the X-ray beam on the image receptor"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 41,
    //     question: "The Coolidge X-ray tube was a significant advancement because it allowed for:",
    //     options: [
    //       "Reduction in scatter radiation",
    //       "Better image quality with lower radiation dose",
    //       "More efficient X-ray production using a heated cathode",
    //       "The invention of computed tomography"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 42,
    //     question: "Which protective barrier should always be used when performing radiographic procedures to limit radiation exposure?",
    //     options: [
    //       "Fluorescent barrier",
    //       "Lead-lined barrier",
    //       "Filtration barrier",
    //       "Intensifying screen"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 43,
    //     question: "In radiologic imaging, what is the main consequence of increasing the mA?",
    //     options: [
    //       "Increased patient dose and increased image brightness",
    //       "Decreased image contrast",
    //       "Increased image contrast with decreased exposure",
    //       "Decreased exposure time"
    //     ],
    //     correctAnswer: 0
    //   },
    //   {
    //     id: 44,
    //     question: "Which scientist is credited with demonstrating the first application of collimation and filtration?",
    //     options: [
    //       "Thomas Edison",
    //       "William Rollins",
    //       "Wilhelm Roentgen",
    //       "Gustav Bucky"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 45,
    //     question: "In digital radiography, the term 'dynamic range' refers to:",
    //     options: [
    //       "The range of X-ray energies used during imaging",
    //       "The range of exposure levels that can produce an acceptable image",
    //       "The ability to change the size of the X-ray field",
    //       "The range of available protective barriers for radiation safety"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 46,
    //     question: "A higher atomic number in target material results in which of the following?",
    //     options: [
    //       "Increased Compton scattering",
    //       "Increased photoelectric interactions",
    //       "Decreased absorption of X-rays",
    //       "Lower radiation dose"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 47,
    //     question: "The primary radiation safety recommendation for technologists during fluoroscopy is to:",
    //     options: [
    //       "Increase kVp",
    //       "Use shielding and minimize exposure time",
    //       "Stand closer to the X-ray source",
    //       "Avoid collimation to capture a larger image area"
    //     ],
    //     correctAnswer: 1
    //   },
    //   {
    //     id: 48,
    //     question: "Which unit is used to measure radiation exposure to biological tissue?",
    //     options: [
    //       "Gray",
    //       "Rem",
    //       "Sievert",
    //       "Rad"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 49,
    //     question: "In radiographic practice, the term ‘scatter radiation’ refers to:",
    //     options: [
    //       "Radiation that passes through the patient without interaction",
    //       "Radiation that is absorbed by the patient’s tissues",
    //       "Radiation that changes direction after interacting with matter",
    //       "Radiation used for fluoroscopy"
    //     ],
    //     correctAnswer: 2
    //   },
    //   {
    //     id: 50,
    //     question: "What is the cardinal rule of radiation protection that emphasizes the importance of keeping the distance from the radiation source as large as possible?",
    //     options: [
    //       "ALARA",
    //       "Inverse square law",
    //       "Radiation threshold",
    //       "Compton effect"
    //     ],
    //     correctAnswer: 1
    //   }
  ];
  