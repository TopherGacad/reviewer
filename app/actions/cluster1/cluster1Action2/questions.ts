export const questions = [
    {
        id: 51,
        question: "What is a positive sign of a problem during evaluation of cassette light leakage test?",
        options: [
          "Black fogging at the edge of the film",
          "Localized blurring",
          "Densities of uncovered and covered side of the film is unequal",
          "Area of higher density"
        ],
        correctAnswer: 0,
        rationale: "Black fogging at the edge of the film indicates a problem with cassette light leakage."
      },
      {
        id: 52,
        question: "Which of the following would suggest that Intensifying screen and film are not fully in close contact with each other?",
        options: [
          "Black fogging at the edge of the film",
          "Localized burning",
          "Densities of uncovered and covered side of the film is unequal",
          "Area of higher density"
        ],
        correctAnswer: 2,
        rationale: "Unequal densities between uncovered and covered sides of the film suggest poor contact between the screen and film."
      },
      {
        id: 53,
        question: "What is the benefit of regularly using wire mesh in quality control?",
        options: [
          "It reduces the radiation dose to patients",
          "It helps in early detection of uniformity problems and resolution deficiencies",
          "It enhances the color contrast of radiographic images",
          "It replaces the need for regular equipment maintenance"
        ],
        correctAnswer: 1,
        rationale: "Wire mesh testing helps detect early uniformity issues and resolution problems."
      },
      {
        id: 54,
        question: "These are equipment with holes with varying densities, used to measure the contrast of an imaging system?",
        options: [
          "Line Pair Phantom",
          "Bar Phantom",
          "Cheese Phantom",
          "Duke Phantom"
        ],
        correctAnswer: 2,
        rationale: "Cheese Phantoms simulate the properties of human tissues and are used for measuring imaging system contrast."
      },
      {
        id: 55,
        question: "Used in Nuclear Medicine to measure spatial resolution.",
        options: [
          "Line Pair Phantom",
          "Bar Phantom",
          "Cheese Phantom",
          "Duke Phantom"
        ],
        correctAnswer: 1,
        rationale: "Bar Phantoms are used in nuclear medicine to measure spatial resolution."
      },
      {
        id: 56,
        question: "What dose-response relationship do cataracts follow?",
        options: [
          "linear, threshold",
          "nonlinear, threshold",
          "nonlinear, nonthreshold",
          "sigmoid curve"
        ],
        correctAnswer: 1,
        rationale: "Cataracts follow a nonlinear-threshold dose-response relationship, which means that they occur only above a certain dose."
      },
      {
        id: 57,
        question: "Biologic effects of radiation that occur relatively soon after humans receive high doses of ionizing radiation are called what?",
        options: [
          "Late effect – low dose, chronic",
          "Somatic effects – damage to tissues or organs",
          "Early effects – high dose, acute",
          "Genetic Effects – damage to reproductive cells"
        ],
        correctAnswer: 2,
        rationale: "Early effects are those that occur after high doses of radiation and show symptoms shortly after exposure."
      },
      {
        id: 58,
        question: "According to their degree of radiosensitivity, which cell type is more sensitive?",
        options: [
          "spermatogonia",
          "nerve cells",
          "chondrocytes",
          "myocytes"
        ],
        correctAnswer: 0,
        rationale: "Spermatogonia are more radiosensitive due to their high rate of cell division and their immature state."
      },
      {
        id: 59,
        question: "Local tissue damage includes effects on ____.",
        options: [
          "Skin and Gonads",
          "Skin and Bone Marrow",
          "Gonads and Bone Marrow",
          "All of these"
        ],
        correctAnswer: 3,
        rationale: "Local tissue damage occurs in skin, gonads, and bone marrow after radiation exposure."
      },
      {
        id: 60,
        question: "Which stage occurs within hours after a whole-body absorbed dose of 1 Gy (100 rad) or more?",
        options: [
          "Gastrointestinal stage",
          "CNS stage",
          "Hematologic stage",
          "Prodromal stage"
        ],
        correctAnswer: 3,
        rationale: "The prodromal stage occurs shortly after exposure to high doses of radiation and includes symptoms such as nausea and vomiting."
      },
      {
        id: 61,
        question: "When cells in the lymphatic system are damaged, the body loses some of its ability to combat what?",
        options: [
          "Infection",
          "Bacteria",
          "Microorganisms",
          "Viruses"
        ],
        correctAnswer: 0,
        rationale: "Damage to the lymphatic system impairs the immune system, reducing the body's ability to fight infection."
      },
      {
        id: 62,
        question: "What is the most severe syndrome with the most severe symptoms?",
        options: [
          "Cerebrovascular syndrome",
          "Gastrointestinal syndrome",
          "Hematologic syndrome",
          "Latent period"
        ],
        correctAnswer: 0,
        rationale: "Cerebrovascular syndrome is the most severe form, often resulting in death within hours to days."
      },
      {
        id: 63,
        question: "What is a clinical sign and symptom in Hematologic Syndrome?  1.Reduction in WBC   2. NVD  3. Fever and infection",
        options: [
          "1 and 2",
          "2 and 3",
          "1 and 3",
          "1, 2 and 3"
        ],
        correctAnswer: 3,
        rationale: "All mentioned symptoms are characteristic of Hematologic Syndrome, which affects blood-forming organs."
      },
      {
        id: 64,
        question: "Which of the following are parts of the law of Bergonie and Tribondeau?",
        options: [
          "A fetus is less radiosensitive than an adult",
          "Younger tissues and organs are more radiosensitive",
          "Mature cells are more sensitive to radiation",
          "High metabolic activity increases radioresistance"
        ],
        correctAnswer: 1,
        rationale: "According to the law of Bergonie and Tribondeau, radiosensitivity is higher in younger tissues and organs."
      },
      {
        id: 65,
        question: "Which of the following conditions shows the longest latent period after exposure to ionizing radiation?",
        options: [
          "Acute Radiation Syndrome (ARS)",
          "Leukemia",
          "Skin Erythema",
          "Epilation"
        ],
        correctAnswer: 1,
        rationale: "Leukemia has a long latent period, often developing years after exposure to ionizing radiation."
      },
      {
        id: 66,
        question: "Which of the following is an early response to radiation exposure?",
        options: [
          "Skin erythema",
          "Cancer",
          "Cataracts",
          "Radiation-induced cardiovascular disease"
        ],
        correctAnswer: 0,
        rationale: "Skin erythema occurs early after high doses of radiation exposure, while the other options are late effects."
      },
      {
        id: 67,
        question: "Which type of radiation unit is used to compare biologic effects for different types of radiation?",
        options: [
          "Air Kerma",
          "Absorbed Dose",
          "Effective Dose",
          "Equivalent Dose"
        ],
        correctAnswer: 3,
        rationale: "Equivalent Dose accounts for the different biologic effects of various types of radiation."
      },
      {
        id: 68,
        question: "Which of the following statements is true regarding dose limits?",
        options: [
          "The general public has a dose limit of 5 mSv annually for infrequent exposure",
          "The lens of the eye has a dose limit of 1 mSv per year",
          "Radiation workers have a dose limit of 1 mSv annually",
          "The hands and feet of the general public have a dose limit of 500 mSv"
        ],
        correctAnswer: 0,
        rationale: "For infrequent exposure, the general public has a dose limit of 5 mSv annually."
      },
      {
        id: 69,
        question: "What is the annual dose limit for the hands of a radiologic technologist?",
        options: [
          "30 mSv",
          "50 mSv",
          "300 mSv",
          "500 mSv"
        ],
        correctAnswer: 3,
        rationale: "The dose limit for the hands of a radiologic technologist is 500 mSv per year."
      },
      {
        id: 70,
        question: "The dose of x-rays necessary to produce erythema in half of those exposed is approximately:",
        options: [
          "50 rad",
          "600 rad",
          "100 rad",
          "300 rad"
        ],
        correctAnswer: 2,
        rationale: "The skin erythema dose (SED50) to affect 50% of exposed individuals is about 100 rad."
      },
      {
        id: 71,
        question: "When an irradiated cell dies before the next mitosis, it is called:",
        options: [
          "Clonal death",
          "Cytogenetic death",
          "Interphase death",
          "Metaphase death"
        ],
        correctAnswer: 2,
        rationale: "Interphase death refers to cell death that occurs during interphase, the phase of the cell cycle between mitotic divisions."
      },
      {
        id: 72,
        question: "A linear, nonthreshold dose-response relationship:",
        options: [
          "Describes most early effects",
          "Has a maximum response followed by a minimum response",
          "Is shaped like an 'S'",
          "Suggests that even the smallest dose may be risky"
        ],
        correctAnswer: 3,
        rationale: "A linear-nonthreshold relationship suggests that any amount of radiation carries a risk of biologic effect."
      },
      {
        id: 73,
        question: "Which of the following describes the combine process of intracellular repair and repopulation?",
        options: [
          "Enzymatic repair",
          "Recovery",
          "Tissue regeneration",
          "Resistance"
        ],
        correctAnswer: 1,
        rationale: "Recovery describes the process of intracellular repair and repopulation after damage."
      },
      {
        id: 74,
        question: "When looking at a graph, this will appear as a straight line that originates from the y-axis above zero:",
        options: [
          "Nonlinear-threshold",
          "Linear-threshold",
          "Nonlinear-nonthreshold",
          "Linear-nonthreshold"
        ],
        correctAnswer: 3,
        rationale: "A linear-nonthreshold graph starts from the y-axis above zero and shows a direct relationship between dose and effect."
      },
      {
        id: 75,
        question: "Of the various macromolecules sensitive to radiation, the most sensitive is/are:",
        options: [
          "DNA",
          "Free radicals",
          "Proteins",
          "Lipids"
        ],
        correctAnswer: 0,
        rationale: "DNA is the most radiosensitive macromolecule due to its central role in cellular function and reproduction."
      },
      {
        id: 76,
        question: "The principal radiation interaction within the human body is with:",
        options: [
          "Fat",
          "Carbohydrates",
          "Water",
          "Proteins"
        ],
        correctAnswer: 2,
        rationale: "Water makes up a large percentage of the human body, and radiation primarily interacts with water molecules."
      },
      {
        id: 77,
        question: "The dose range needed to induce permanent sterility in both ovaries is:",
        options: [
          "10-100 rad",
          "100-200 rad",
          "300-800 rad",
          "900-1200 rad"
        ],
        correctAnswer: 2,
        rationale: "A dose of 300-800 rad is required to induce permanent sterility."
      },
      {
        id: 78,
        question: "Which of the following is NOT considered an early response to radiation exposure?",
        options: [
          "Breast cancer",
          "Epilation",
          "Intestinal distress occurring 1 week after exposure",
          "Skin erythema occurring 2 weeks after exposure"
        ],
        correctAnswer: 0,
        rationale: "Breast cancer is a late effect, not an early response to radiation exposure."
      },
      {
        id: 79,
        question: "The latent period for radiation-induced leukemia is considered to be:",
        options: [
          "1-3 years",
          "4-7 years",
          "8-12 years",
          "More than 12 years"
        ],
        correctAnswer: 1,
        rationale: "The latent period for radiation-induced leukemia is typically 4-7 years after exposure."
      },
      {
        id: 80,
        question: "The annual dose limit for the hands of a radiologic technologist is:",
        options: [
          "30 mSv",
          "50 mSv",
          "300 mSv",
          "500 mSv"
        ],
        correctAnswer: 3,
        rationale: "The dose limit for the hands of a radiologic technologist is 500 mSv per year."
      },
      {
        id: 81,
        question: "The latent period of radiation-induced leukemia is considered to be:",
        options: [
          "1-3 years",
          "4-7 years",
          "8-12 years",
          "> 12 years"
        ],
        correctAnswer: 1,
        rationale: "Radiation-induced leukemia has a latent period of 4-7 years and an at-risk period of approximately 20 years."
      },
      {
        id: 82,
        question: "Which of the following radiation units compares the biologic effects for different types of radiation?",
        options: [
          "Air Kerma",
          "Absorbed Dose",
          "Effective Dose",
          "Equivalent Dose"
        ],
        correctAnswer: 3,
        rationale: "Equivalent Dose is used to compare the biologic effects of different types of radiation."
      },
      {
        id: 83,
        question: "What is the annual dose limit for the general public with infrequent exposure?",
        options: [
          "1 mSv",
          "5 mSv",
          "15 mSv",
          "50 mSv"
        ],
        correctAnswer: 1,
        rationale: "The annual dose limit for the general public with infrequent exposure is 5 mSv."
      },
      {
        id: 84,
        question: "The annual dose limit for the hands of a radiologic technologist is:",
        options: [
          "30 mSv",
          "50 mSv",
          "300 mSv",
          "500 mSv"
        ],
        correctAnswer: 3,
        rationale: "The dose limit for the hands of a radiologic technologist is 500 mSv per year."
      },
      {
        id: 85,
        question: "Which of the following is a source of secondary radiation?",
        options: [
          "X-rays",
          "Scatter radiation",
          "Leakage radiation",
          "All of the above"
        ],
        correctAnswer: 3,
        rationale: "Secondary radiation includes scatter radiation, leakage radiation, and radiation from any object that has been exposed to x-rays."
      },
      {
        id: 86,
        question: "Which of the following conditions shows the longest latent period after exposure to ionizing radiation?",
        options: [
          "Acute Radiation Syndrome (ARS)",
          "Leukemia",
          "Skin Erythema",
          "Epilation"
        ],
        correctAnswer: 1,
        rationale: "Leukemia has a long latent period, often developing years after exposure to ionizing radiation."
      },
      {
        id: 87,
        question: "Which stage occurs within hours after a whole-body absorbed dose of 1 Gy (100 rad) or more?",
        options: [
          "Gastrointestinal stage",
          "CNS stage",
          "Hematologic stage",
          "Prodromal stage"
        ],
        correctAnswer: 3,
        rationale: "The prodromal stage occurs shortly after exposure to high doses of radiation and includes symptoms such as nausea and vomiting."
      },
      {
        id: 88,
        question: "What is the most severe syndrome with the most severe symptoms?",
        options: [
          "Cerebrovascular syndrome",
          "Gastrointestinal syndrome",
          "Hematologic syndrome",
          "Latent period"
        ],
        correctAnswer: 0,
        rationale: "Cerebrovascular syndrome is the most severe form, often resulting in death within hours to days."
      },
      {
        id: 89,
        question: "What dose-response relationship do cataracts follow?",
        options: [
          "Linear, threshold",
          "Nonlinear, threshold",
          "Nonlinear, nonthreshold",
          "Sigmoid curve"
        ],
        correctAnswer: 1,
        rationale: "Cataracts follow a nonlinear-threshold dose-response relationship, meaning they occur only above a certain dose."
      },
      {
        id: 90,
        question: "Which of the following materials is used to protect against radiation exposure?",
        options: [
          "Concrete",
          "Lead",
          "Steel",
          "All of the above"
        ],
        correctAnswer: 3,
        rationale: "Lead, concrete, and steel are materials used to shield against radiation exposure."
      },
      {
        id: 91,
        question: "Which of the following is the similarity found in the operation of scintillation detectors and solid-state detectors?",
        options: [
          "Both are using air, and argon as measuring medium.",
          "Both are operated by the use of high voltage transformer",
          "Both are current dependent.",
          "Both operate by measuring the number of liberated electrons."
        ],
        correctAnswer: 3,
        rationale: "Both scintillation and solid-state detectors operate by measuring the number of liberated electrons when interacting with radiation."
      },
      {
        id: 92,
        question: "What is the active material of a film badge?",
        options: [
          "Aluminum",
          "Film",
          "Lithium fluoride",
          "Aluminum oxide"
        ],
        correctAnswer: 1,
        rationale: "Film is the active material in a film badge used for personnel radiation monitoring."
      },
      {
        id: 93,
        question: "Which of the following personnel radiation monitors is most sensitive?",
        options: [
          "Film badge",
          "Geiger-Mueller tube",
          "Scintillation detector",
          "TLD device"
        ],
        correctAnswer: 3,
        rationale: "TLD devices are the most sensitive radiation monitors, detecting doses as low as 50 μGya."
      },
      {
        id: 94,
        question: "Which of the following is an advantage of film over TLD for personnel monitoring?",
        options: [
          "Can be reused",
          "Can be used for longer periods",
          "Is cheaper",
          "Is less energy-dependent in the diagnostic range"
        ],
        correctAnswer: 2,
        rationale: "Film badges are generally cheaper compared to TLD devices."
      },
      {
        id: 95,
        question: "Filters are used in film badges to:",
        options: [
          "Correct for film fog.",
          "Correct for length of wear.",
          "Estimate radiation energy.",
          "Increase sensitivity to specific radiation."
        ],
        correctAnswer: 2,
        rationale: "Filters in film badges help estimate the energy of the radiation by analyzing the degree of luminescence."
      },
      {
        id: 96,
        question: "Which of the following materials can be used as an alternative to lead in a primary barrier?  (1.) Concrete  (2.) Steel  (3.) Gypsum   (4.) Wood",
        options: [
          "1 only",
          "1 and 2",
          "1, 2 and 3",
          "1, 2, 3 and 4"
        ],
        correctAnswer: 3,
        rationale: "Concrete, steel, and gypsum can all be used as materials for primary barriers in radiation protection."
      },
      {
        id: 97,
        question: "If a radiation therapy unit delivers a dose rate of 2 Gy/min, how much dose will be delivered in 15 minutes?",
        options: [
          "20 Gy",
          "25 Gy",
          "30 Gy",
          "35 Gy"
        ],
        correctAnswer: 2,
        rationale: "At a dose rate of 2 Gy/min, the total dose delivered in 15 minutes is 30 Gy."
      },
      {
        id: 98,
        question: "A point source of radiation has a dose rate of 10 mGy/hr at 1 meter. What will be the dose rate at 3 meters?",
        options: [
          "1.11 mGy/hr",
          "3.33 mGy/hr",
          "5.55 mGy/hr",
          "7.77 mGy/hr"
        ],
        correctAnswer: 0,
        rationale: "According to the inverse square law, the dose rate at 3 meters will be 1.11 mGy/hr."
      },
      {
        id: 99,
        question: "A linear accelerator has a dose rate of 6 Gy/min at a specific point. If the treatment time is 5 minutes, what is the total dose delivered?",
        options: [
          "20 Gy",
          "25 Gy",
          "30 Gy",
          "35 Gy"
        ],
        correctAnswer: 2,
        rationale: "At a dose rate of 6 Gy/min, the total dose delivered in 5 minutes is 30 Gy."
      },
      {
        id: 100,
        question: "If the dose rate from a radiation source is 80 mGy/hr at 2 meters, what will be the dose rate at 4 meters?",
        options: [
          "10 mGy/hr",
          "20 mGy/hr",
          "40 mGy/hr",
          "80 mGy/hr"
        ],
        correctAnswer: 0,
        rationale: "Using the inverse square law, the dose rate at 4 meters will be reduced to 10 mGy/hr."
      }
      
      
      
      
]